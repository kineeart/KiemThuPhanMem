  ...
  catchAsync(async (req, res, next) => {
    if (Model == Product) {
      ...
      if (req.body.promotion > req.body.price)
        return next(new AppError("Giá giảm không được lớn hơn giá gốc", 500));
      if (req.body.inventory < 0)
        return next(new AppError("Số lượng tồn kho phải là số dương", 400));
      if (req.body.inventory > 1000)
        return next(
          new AppError("Số lượng tồn kho không được vượt quá 1000", 400)
        );
      if (req.body.weight < 0)
        return next(new AppError("Trọng lượng phải là số dương", 400));
      if (req.body.weight >= 1000)
        return next(new AppError("Trọng lượng phải nhỏ hơn 1000", 400));
